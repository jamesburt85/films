...
//} elseif ( ('work' == get_post_type()) || is_tax('work_category') ) {
    //$taxonomy   = $showTax ?? 'work_category';
} elseif ( ('work' == get_post_type()) || is_tax('work_department') ) {
    $taxonomy   = $showTax ?? 'work_department';
} else {
  $taxonomy   = $showTax ?? 'kit_category';
}
// $orderby      = 'name'; 
...
<div class="flow--medium">
  ...
  <div>
    <ul class="category-list__wrapper category-list flex">
      ...
      <li class="">
        <a href="<?php echo get_post_type_archive_link($pType); ?>" class="<?php if( is_post_type_archive() ) { echo " active "; } ?>pill">
          All
        </a>
      </li>
      <?php
        ...
        foreach($categories as $category) {
          ...
          echo '<li class="pill ' . $class . '">
            <a class="" href="' . get_category_link( $category->term_id ) . '" title="' . sprintf( __( "View all posts in %s" ), $category->name ) . '" ' . '>' . $category->name.'</a>
            </li>';
          }

        if ( 'people' == get_post_type() ) {
          echo '<div class="pill--secondary__item"><li class="pill pill--secondary">
            <a class="" href="' . get_site_url() . '/freelancers" title="">Freelancers</a>
            </li></div>';
          }
